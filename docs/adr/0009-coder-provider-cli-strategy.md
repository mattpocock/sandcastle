...
- File copy uses OpenSSH `ssh` streams with a per-command `ProxyCommand=coder ssh --stdio ...` so transfer still goes through the Coder CLI tunnel without mutating the user's SSH config. Direct `coder ssh -- sh -c ...` remains used for command execution and setup commands.
