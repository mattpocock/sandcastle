...
- **Linear** installs `@schpet/linear-cli`, lists non-completed issues with the `Sandcastle` label via `linear issue query`, views issues with `linear issue view`, and completes them by adding a comment then updating the issue state to `completed`.

GitHub Issues and Linear both offer to create the `Sandcastle` label during init. If you decline, the generated prompts remove the `--label Sandcastle` filter.

For Linear, set `LINEAR_API_KEY` in `.sandcastle/.env` or configure `linear-cli` credentials separately. In multi-workspace setups, set `LINEAR_WORKSPACE`. To use a different completion state, set `LINEAR_DONE_STATE`.
...
- Backlog manager tooling for the selected backlog manager, such as **GitHub CLI** (`gh`), **Beads** (`bd`), or **Linear CLI** (`linear`)
- **Claude Code CLI**
